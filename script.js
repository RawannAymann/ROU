/* script.js */
const PRODUCTS = [
  { id: 1, name: "Backstage Glow Maximizer Face Palette", category: "face", price: 23.00, imageUrl: "https://www.sephora.com/productimages/sku/s2882553-main-zoom.jpg?imwidth=1224", featured: true, brand: "Dior" },
  { id: 2, name: "Make Me Blush 24H Buildable Powder Blush", category: "face", price: 48.00, imageUrl: "https://www.sephora.com/productimages/sku/s2935195-main-zoom.jpg?imwidth=1224", featured: true, brand: "Yves Saint Laurent" },
  { id: 3, name: "Make Me Blush 12H Blurring Liquid Blush", category: "lip", price: 16.00, imageUrl: "https://www.sephora.com/productimages/sku/s2872182-main-zoom.jpg?imwidth=1224", featured: true, brand: "Yves Saint Laurent" },
  { id: 4, name: "Huda Beauty Easy Bake Loose Powder", category: "face", price: 38.00, imageUrl: "https://www.sephora.com/productimages/sku/s2934230-main-zoom.jpg?imwidth=1224", featured: false, brand: "Huda Beauty" },
  { id: 5, name: "Mini Easy Blur Silicone-Free Smoothing Primer", category: "face", price: 31.00, imageUrl: "https://www.sephora.com/productimages/sku/s2888832-main-zoom.jpg?imwidth=1224", featured: true, brand: "Huda Beauty" },
  { id: 6, name: "Lash Clash Extreme Volume Mascara", category: "eyes", price: 28.00, imageUrl: "https://www.sephora.com/productimages/sku/s2638336-main-zoom.jpg?imwidth=1224", featured: true, brand: "Yves Saint Laurent" },
  { id: 7, name: "Couture Mini Clutch Eyeshadow Palette", category: "face", price: 31.00, imageUrl: "https://www.sephora.com/productimages/sku/s2873453-main-zoom.jpg?imwidth=1224", featured: true, brand: "Yves Saint Laurent" },
  { id: 8, name: "YSL Candy Glaze Lip Gloss Stick", category: "face", price: 36.00, imageUrl: "https://www.sephora.com/productimages/sku/s2511335-main-zoom.jpg?imwidth=1224", featured: true, brand: "Yves Saint Laurent" },
  { id: 9, name: "YSL Loveshine Plumping Lip Oil Gloss", category: "fragrance", price: 275.00, imageUrl: "https://www.sephora.com/productimages/sku/s2830172-main-zoom.jpg?imwidth=1224", featured: true, brand: "Yves Saint Laurent" },
  { id: 10, name: "Backstage Face & Body Foundation", category: "body", price: 48.00, imageUrl: "https://www.sephora.com/productimages/sku/s2669539-main-zoom.jpg?imwidth=1224", featured: true, brand: "Dior" }
];
...
function saveCart() { localStorage.setItem("rou_cart", JSON.stringify(cart)); }
function loadCart() { const stored = localStorage.getItem("rou_cart"); cart = stored ? JSON.parse(stored) : []; updateCartCount(); }
...
function addToCart(product, qty = 1) {
  ...
  if (existing) {
    ...
    cart.push({ ...product, quantity: qty, imageUrl: product.imageUrl });
  }
  saveCart();
  updateCartCount();
...
function renderProductsGrid(productList, btnText = "Add to Cart") {
  ...
  return `<div class="products">` + productList.map(prod => {
    const imageHtml = prod.imageUrl ? `<img src="${prod.imageUrl}" alt="${prod.name}">` : `<span style="font-size:4rem;">💄</span>`;
    return `
      <div class="product-card">
        ...
        <div class="product-info">
          ...
          <button class="add-to-cart" data-id="${prod.id}" data-name="${escapeHtml(prod.name)}" data-price="${prod.price}" data-image="${prod.imageUrl || ''}">${btnText}</button>
        </div>
      </div>
    `;
  }).join('') + `</div>`;
  ...
  let cartItemsHtml = cart.map(item => `
    <div class="cart-item">
      <div style="display:flex; align-items:center; gap:1rem;">
        ${item.imageUrl ? `<img src="${item.imageUrl}" alt="${item.name}">` : `<span style="font-size:2rem;">💄</span>`}
        <div>
...
function updateSidebarActiveLink() {
  document.querySelectorAll('.sidebar-nav a').forEach(link => {
    ...
    if (navValue === currentView) link.classList.add('active');
...
function attachGlobalEvents() {
  document.body.addEventListener('click', (e) => {
    ...
    if (e.target.classList.contains('shop-now-btn')) navigateTo('products');
    ...
    if (removeBtn && removeBtn.dataset.id) removeCartItem(parseInt(removeBtn.dataset.id));
    
    const checkoutBtn = e.target.closest('#checkout-btn');
    if (checkoutBtn) {
      if (cart.length) {
        showToast("🛍️ Proceeding to secure checkout (demo)");
        setTimeout(() => alert("Thank you for shopping at ROU BEAUTY! (Demo checkout)"), 100);
